...
root = ET.parse('filelist/files.xml').getroot()
with open("filelist/small/train_list.txt", 'w', encoding='utf-8') as out_f:
    with open("filelist/small/val_list.txt", 'w', encoding='utf-8') as val_f:
        for file in root.findall("file"):
            value = file.get("name")
            for fragment in file.findall("fragment"):
